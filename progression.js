...
// Call when a set/exercise is intentionally skipped for time, not pain or
// difficulty. This is neutral: it doesn't advance progression and doesn't
// count against it either, but flags a catch-up note for next session.
function logSkip(person, exId){
  const p = readProgress();
  const st = p[person][exId];
  if(!st) return null;
  st.catchUpOwed = (st.catchUpOwed||0) + 1;
  saveProgress(p);
  return st;
}
...
function initExerciseState(person, exId, startWeight, repsLow, repsHigh){
  ...
  if(!p[person][exId]){
    p[person][exId] = {
      ...
      catchUpOwed: 0        // count of skipped sessions not yet made up
    };
    saveProgress(p);
  }
...
function logProgressSession(person, exId, weight, s1, s2, feel, dateStr){
  ...
  // A completed session clears any owed catch-up from a prior skip.
  if(st.catchUpOwed) st.catchUpOwed = 0;
...
function evaluateReadiness(person, exId){
  ...
  if(st.catchUpOwed>0){
    return {action:'catchup', message:'You skipped this last time to save time — no penalty, but try to get a full set in today if you can.'};
  }
...
if (typeof module !== 'undefined') module.exports = {
  initExerciseState, getExerciseState, logProgressSession, evaluateReadiness, applyWeightIncrease, logSkip,
